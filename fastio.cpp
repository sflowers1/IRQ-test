...
//   cout << "Starting the LED2 program" << endl;
  // cout << "The LED Path is: " << LED0_PATH << endl;   
   ...
   iolib_init(); 
   iolib_setdir(1, 15, DIR_OUT);	// timer output
   iolib_setdir(1, 16, DIR_IN);		// input 
   iolib_setdir(0, 26, DIR_OUT);	// toggle output
...
//		perror("setitmer");
   ...
   fdset.fd = open("/sys/class/gpio/gpio46/value", O_RDONLY | O_NONBLOCK);
